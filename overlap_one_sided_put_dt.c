...
int main(void)
{
  int i,me,np,n,provided,tmp,*blk_len,*disp,name_len;
  ...
  MPI_Win_allocate(win_size, 1, MPI_INFO_NULL, MPI_COMM_WORLD, &y, &win);
  ...
  if(me == 0)
    {
      ...
      MPI_Win_lock(MPI_LOCK_EXCLUSIVE, np-1, 0, win);
      MPI_Put(x, 1, dt, np-1, 0, 1, dt, win);
      MPI_Win_unlock(np-1,win);
      end_time = MPI_Wtime();
